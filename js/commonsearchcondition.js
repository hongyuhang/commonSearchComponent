...
//				"id"	 : "id",
//				"name" : "name",
//				"type" : "text, checkbox, select, radio, date", 
//				"placeHolder" : "",
//				"need" : "",
//				"value" : "",
	...
	this._registHandle = function() {
		...
		$.each($('#conditionLists').children(), function(i, obj) {
			$(obj).click(function(){
				...
				// 如果是可选项或者多选项的情况下
				if (conditionObj.type == "select" || conditionObj.type == "multiSelect") {
					var multi = "";
					if (conditionObj.type == "multiSelect") {
						multi = "multiple";
					}
				}
...
//          },
	...
	this._relationUpdate = function() {
		...
		if (label.length == 0 && value != "") {
			// 向展示区域添加label
			label = $('<span style="margin-right:10px;display:inline-block;cursor:pointer;" hIndex="' + index + '" class="label label-info">' + condition.labelText + ':' + condition.value + '<a href="#"><span class="glyphicon glyphicon-remove" style="color:black;" aria-hidden="true"></span></a></span>');
			// 注册清除按钮的动作
			label.find('a').click(function(event){
				// 如果要清除的条件正好是通用搜索框的条件的话，通过改变搜索框的值来触发清除事件
				if ($(this).parent().attr('hIndex') == $('#commonSearchInput').attr('hIndex')) {
					$('#commonSearchInput').val("");
					$('#commonSearchInput').trigger("keyup");
				} else {
					self.options.conditions[$(this).parent().attr('hIndex')].value = "";
					$(this).parent().remove();
					$('#commonSearchInput').trigger("keyup");
				}
		...
		if (label.length > 0 && value != "") {
			// 修改已有的label信息
			label.html(condition.labelText + ':' + condition.value + '<a href="#"><span class="glyphicon glyphicon-remove" style="color:black;" aria-hidden="true"></span></a>');
			// 注册清除按钮的动作
			label.find('a').click(function(event){
				// 如果要清除的条件正好是通用搜索框的条件的话，通过改变搜索框的值来触发清除事件
				if ($(this).parent().attr('hIndex') == $('#commonSearchInput').attr('hIndex')) {
	...
	this._createSearchToolBar = function() {
		...
		// 通用搜索框(TODO:需要考虑select,checkbox,radio的情况)
